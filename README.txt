...
All the important files containing the website's content are in .content.fUWbb4hv folder. Its name starts with a period for security purposes, so that it is not displayed in the list of folders located on the server when the directory listing is opened.

ATTENTION!!!
In case you do not see the folder named .content.fUWbb4hv or the file named .htaccess, you are possibly using MacOS which hides folders and files whose names start with a period by default. To see them, press Cmd+Shift+. (period) on your keyboard.
In case you use a Unix terminal, you use the command `ls -a` (with the parameter -a), to see this folder and the files.
 ...
 * It is sufficient to upload the .content.fUWbb4hv folder and the index.php and the .htaccess files to the hosting server's root directory


NGINX SERVER WITH PHP
---------------------
*Make sure the server has PHP 5.6 or above installed on it. You also must have the PDO_SQLITE extension activated (it is named simply pdo for PHP 5, and pdo_sqlite for PHP 7)
  * It is sufficient to upload the .content.fUWbb4hv folder and the index.php file to the hosting server's root directory.
  * You need to add the following rule to the NGINX configuration:
      try_files $uri $uri/ /index.php;
   This rule will direct all requests to index.php
...
The principle of integration with Wordpress is as follows:
    ...
    2. Rename the index.php script, using any name, for example, archivarix.php
...
ARCHIVARIX_INCLUDE_CUSTOM
  has parameters:
    FILE - the name of the file that you place in .content.fUWbb4hv folder,
...
ARCHIVARIX_CONTENT_PATH
  if you have renamed the .content.fUWbb4hv folder, specify the new name in this parameter. By default, the script automatically searches for a folder of the .content.xxxxxxxx type in its directory and uses the first found one.
...
Все файлы с содержимым сайта находятся в папке .content.fUWbb4hv. Она начинается с точки по причинам безопасности, чтобы не отображаться в списке папок на сервере при открытом листинге директории.

ВНИМАНИЕ!!! Если вы не видите папку с названием .content.fUWbb4hv или файла .htaccess, вероятнее всего вы используете операционную систему MacOS, которая по-умолчанию скрывает файлы и папки, начинающиеся с точки. Чтобы увидеть их, нажмите на клавиатуре комбинацию клавиш Cmd+Shift+. (точка).
 ...
 * На хостинг в корень достаточно загрузить папку .content.fUWbb4hv, а так же файлы index.php и .htaccess


СЕРВЕР NGINX С PHP
------------------
 * Убедитесь, что на сервере установлен PHP версии 5.6 или выше и php расширение PDO_SQLITE (для PHP 5-ой версии он просто называется pdo, для PHP 7-ой версии pdo_sqlite)
 * На хостинг в корень достаточно загрузить папку .content.fUWbb4hv, а так же файл index.php
 * В конфигурации NGINX необходимо добавить правило:
    try_files $uri $uri/ /index.php;
   которое будет направлять все запросы на index.php
...
Принцип интеграции с Wordpress:
    ...
    2. Переименуйте наш скрипт index.php на любой другое название, например, archivarix.php
    3. В php-код index.php от Wordpress, добавьте в самое начало (после тега <?php) строку:
        include __DIR__ . DIRECTORY_SEPARATOR . 'archivarix.php';
Данное решение позволит отображать на сайте полноценную рабочую версию старого сайта и одновременно добавлять новые страницы в вашей любимой CMS.


ПАРАМЕТРЫ index.php
-------------------
ARCHIVARIX_CMS_MODE
  включает режим CMS, когда наш php скрипт живет совместно с вашей установленной CMS и ничего не делает (передает управление CMS) если запрашиваемого адреса нет среди восстановленных. При включении этого режима автоматически отключаются последующие 5 параметров (*MISSING*). Инструкцию по интеграции читайте выше.
...
ARCHIVARIX_INCLUDE_CUSTOM
  имеет параметры:
    FILE - название подгружаемого файла, который нужно расположить в папке .content.fUWbb4hv,
...
ARCHIVARIX_CONTENT_PATH
  если вы сменили название папки .content.fUWbb4hv, тогда укажите его в этом параметре. По-умолчанию скрипт автоматически ищет папку вида .content.xxxxxxxx в своей директории и использует первую найденную
...
Todos los archivos con el contenido del sitio se encuentran en la carpeta .content.fUWbb4hv. Empieza por un punto por motivos de seguridad, para no reflejarse en la lista de carpetas en el servidor con el listado abierto del directorio.

¡¡¡ATENCION!!! Si no ve la carpeta con el nombre .content.fUWbb4hv o el archivo .htaccess, lo más seguro es que esté usando el sistema operativo MacOS, el cual por defecto oculta los archivos y las carpetas que empiezan por un punto. Para verlos, pulse en el teclado la combinación de teclas Cmd+Shift+. (punto).
 ...
 * En el alojamiento web, en la raíz, es suficiente con cargar la carpeta .content.fUWbb4hv, así como los archivos index.php y .htaccess
	
SERVIDOR NGINX CON PHP
----------------------
 * Asegúrese de que en el servidor esté instalado PHP versión 5.6 o superior y la extensión php PDO_SQLITE (para PHP versión 5 se llama simplemente pdo, para PHP versión 7 pdo_sqlite)
 * En el alojamiento web, en la raíz, es suficiente con cargar la carpeta .content.fUWbb4hv, así como el archivo index.php
 * En la configuración NGINX hay que añadir la regla:
    try_files $uri $uri/ /index.php;
   que redirigirá todas las solicitudes a index.php
...
ARCHIVARIX_INCLUDE_CUSTOM
  tiene parámetros:
    FILE - nombre del archivo que se carga, que hay que ubicar en la carpeta .content.fUWbb4hv,
...
ARCHIVARIX_CONTENT_PATH
  si ha cambiado el nombre de la carpeta .content.fUWbb4hv, entonces indíquelo en este parámetro. Por defecto el script busca automáticamente la carpeta de tipo .content.xxxxxxxx en su directorio y usa la primera encontrada
